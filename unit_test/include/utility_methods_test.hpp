...
TEST_F(UtilityMethodsTest, DifferenceOfTwoStates)
{
  ...
  ASSERT_FLOAT_EQ( std::norm(psi_a.ComputeOverlap(psi_a)), 1. );
  ASSERT_FLOAT_EQ( std::norm(psi_b.ComputeOverlap(psi_a)), 0. );
  ASSERT_FLOAT_EQ(psi_b.MaxAbsDiff(psi_a), 1. );
  ...
  if ( psi_b.LocalSize() > index )
      ASSERT_FLOAT_EQ(psi_b.MaxL2NormDiff(psi_a), 2. );
  else
      ASSERT_FLOAT_EQ(psi_b.MaxL2NormDiff(psi_a), 1. );

  psi_b.ApplyHadamard(2);
  // |a> = |0000>
  // |b> = |00-0>
  ASSERT_FLOAT_EQ( std::norm(psi_b.ComputeOverlap(psi_a)), 0.5 );
  ASSERT_FLOAT_EQ( psi_b.ComputeOverlap(psi_a).real(), inv_sqrt_2 );
  ASSERT_FLOAT_EQ(psi_b.MaxAbsDiff(psi_a), inv_sqrt_2 );
  // Note that the only non-zero amplitudes have index 0 and 4, for both |a> and |b>.
  if ( psi_b.LocalSize() > 4 )
      ASSERT_FLOAT_EQ(psi_b.MaxL2NormDiff(psi_a), (1-inv_sqrt_2)*(1-inv_sqrt_2)+0.5 );
  else
      ASSERT_FLOAT_EQ(psi_b.MaxL2NormDiff(psi_a),
  ...
  // |a> =  |0011> =  |"12">
  // |b> = -|0011> = -|"12">
  ASSERT_FLOAT_EQ( psi_b.ComputeOverlap(psi_a).real(), -1. );
  ASSERT_FLOAT_EQ(psi_b.MaxL2NormDiff(psi_a), 4. );
  ...
  if ( psi_b.LocalSize() > 12 )
      ...
      ASSERT_FLOAT_EQ(psi_b.MaxL2NormDiff(psi_a), 3. );
