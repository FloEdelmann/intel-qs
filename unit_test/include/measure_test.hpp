...
TEST_F(MeasureTest, GetProbability)
{
  ...
  psi.ApplyHadamard(3);
  // |psi> = |01+-> = H2.H3|"2+8">
  ASSERT_FLOAT_EQ(psi.GetProbability(0), 0.);
  ASSERT_FLOAT_EQ(psi.GetProbability(1), 1.);
  ASSERT_FLOAT_EQ(psi.GetProbability(2), 0.5);
  ASSERT_FLOAT_EQ(psi.GetProbability(3), 0.5);
