...
int main(int argc, char * argv[])
{
    ...
    MPI_Count c = n/sizeof(float);

    float * baseptr = NULL;
    MPI_Win win;
    /* Allocate all the window memory on rank 0 */
    MPI_Win_allocate((MPI_Aint)(rank==0 ? n : 0), sizeof(float), MPI_INFO_NULL, MPI_COMM_WORLD, &baseptr, &win);
    MPI_Win_lock_all(0, win);

    if (rank==0) {
        set_floats(baseptr, c, 0.0);
        MPI_Win_sync(win);
    }
    ...
    if ((size==1 && rank==0) || rank==1) {
        ...
        set_floats(buf, c, 7.0);
        MPIX_Put_x(buf, c, MPI_FLOAT, 0 /* target */, 0 /* disp */, c, MPI_FLOAT, win);
        set_floats(buf, c, 17.0);
        MPIX_Accumulate_x(buf, c, MPI_FLOAT, 0 /* target */, 0 /* disp */, c, MPI_FLOAT, MPI_SUM, win);
        set_floats(buf, c, 0.0);
        MPIX_Get_x(buf, c, MPI_FLOAT, 0 /* target */, 0 /* disp */, c, MPI_FLOAT, win);

        float expected = 7.0 + 17.0;
        size_t errors = verify_floats(buf, c, expected);
        if (errors > 0) {
